...
const [name,setName] = useState(route.params.params.request_message)
...
return (
    ...
    <Text>  </Text> 
    {/* <TouchableOpacity activeOpacity={0.5} onPress={()=>navigation.navigate('Record',{'request_id':route.params.request_id})}>
    ...
    <TextInput style={[styles.inputStyle,{marginLeft:40}]} placeholder={'VideoName'} underlineColorAndroid={'transparent'} keyboardType={'numeric'} value={name} onChangeText={text=>{setName(text)}}/>
    </View>

    <Text>  </Text>
    <View style={{flexDirection:'row'}}> 
    <Text style={[styles.textstyle,{borderWidth:0}]}> Keywords: </Text>
    <TextInput style={styles.subTextinStyle} placeholder={'Keyword 1'} underlineColorAndroid={'transparent'} keyboardType={'numeric'} onChangeText={text=>{setKey1(text)}} keyboardType={'email-address'}/>
    <TextInput style={styles.subTextinStyle} placeholder={'Keyword 2'} underlineColorAndroid={'transparent'} keyboardType={'numeric'} onChangeText={text=>{setKey2(text)}} keyboardType={'email-address'}/>
    <TextInput style={styles.subTextinStyle} placeholder={'Keyword 3'} underlineColorAndroid={'transparent'} keyboardType={'numeric'} onChangeText={text=>{setKey3(text)}} keyboardType={'email-address'}/>
    </View>
    ...
    <Text style={[styles.textstyle]}> Permissions: </Text>
      <RadioGroup onSelect = {(index, value) => setPrivat(index)} style={{flexDirection:'row'}} selectedIndex={1}>
...
*/
        var data = {
            ...
            response_id:route.params.params.response_id,
            request_id:route.params.params.request_id?route.params.params.request_id:''
        }
        ...
        if(data.video_name==""&&data.short_description==""&&data.keyword1==""&&data.keyword2==""&&data.keyword2==""&&data.keyword3==""){
            ...
            data['requestor_id'] = route.params.params.requestor_id

            

            var result = await request.post('/front-end/addVideo',data)
            await axios.get(`${SERVER_ADDRESS}/front-end/addUserExperience/${RootStore.UserId}`)
            setTime(0)
            if(route.params.params.response_id=='new'){
              navigation.navigate('Search')
            }else{
              await axios.get(`${SERVER_ADDRESS}/front-end/readResponse/${route.params.params.response_id}`)
