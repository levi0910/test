...
const Post = (props) => {
	...
	const PostComment = async()=>{
        ...
        await fetch('http://192.168.2.94:3000/front-end/comment', {
			...
			var result = await request.get('/front-end/getComment/58f5ffcf-716b-11eb-a09f-f0795907d9ec')
	...
	const onLikePress = () => {
		...
		fetch('http://192.168.2.94:3000/front-end/like', {
			method: 'POST',
			headers: {
				//Accept: 'application/json',
				'Content-Type': 'application/json'
					...
					<View style={styles.centeredView}>
						<View style={styles.modalView}>
							...
							<View style={{flex: 1, backgroundColor: '#FFF'}}>
								<ScrollView
								>	
									{content.map((value,index)=><Comment key={index} username={value.username} 
									content={value.content} uri={'http://'+value.BaseUri+value.photopath} />)}
									
								</ScrollView>

								{/*keyboard input start*/}
								<Input closeModal={setModalVisible} PostComment={PostComment} setComment={ChangeText}/>
								{/*keyboard input end */}
							</View>
						</View>
					</View>
